...
void startOver() {
  ...
  //for (int i = 0; i < xoSpaces.length-5; i++) {
  //  if (random(1) < .5)
  //    xoSpaces[i] = huPlayer;
  //  else
  //    xoSpaces[i] = aiPlayer;
  //}
  int[] indexes = emptySquares(xoSpaces);
  for (int i : indexes)
    xoSpaces[i] = "N";
...
int bestSpot() {
  ...
  //return emptySquares(xoSpaces)[int(random(emptySquares(xoSpaces).length)];
}
...
int[] emptySquares(String[] board) {
  ...
  for (int i = 0; i < board.length; i++) {
    if (xoSpaces[i]==null || xoSpaces[i].equals("N"))
...
boolean checkIfTie() {
  ...
  //else
  //  return false;
  int len = emptySquares(xoSpaces).length;
  if (len > 0)
    return false;
  else
    return true;
